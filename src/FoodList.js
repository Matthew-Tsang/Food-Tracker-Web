...
function FoodList() {
    ...
    useEffect(() =>
    {
        ...
        const FoodList = () =>
        {
            if(!items || !items.foodItems)
            {
                return (
                    <div>
                        <Sidebar />
                    <div id="foodlist-page-container">
                        <h2>Create Food Items</h2>
                        <input type="text" placeholder="Search Anything" id="search-input"></input>
                        <div id="foodlist-wrapper">
                            <ul id="foodlist"></ul>
                        </div>
                        <button class="button" id="create-fooditem-btn" onClick={() => handleRouting("/createfood")}>Create Food Item</button>
                    </div>
                    </div>
                )
            } else {
                    ...
                    <div id="foodlist-page-container">
                        ...
                        <div id="foodlist-wrapper">
                            ...
                            {items.foodItems.map((item) => {
                                return <FoodListItem name={item.name} quantity={item.grams} key={count++} carbs={item.carbs} protein={item.protein} kcal={item.calories} cholesterol={item.cholesterol} fat={item.fat} fiber={item.fiber} sodium={item.sodium} sugar={item.sugar} foodId={item.id} cost={item.cost} listId={items.id} isPlan={false}/>;
                            })}
                            </ul>
                        </div>
                        <button class="button" id="create-fooditem-btn" onClick={() => handleRouting("/createfood")}>Create Food Item</button>
